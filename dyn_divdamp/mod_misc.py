...
data = np.fromfile("dyn_divdamp/vgrid40_600m_24km.dat", dtype=np.dtype('>f8'))

# for i in range(len(data)):
#     print(data[i])

def GRD_input_vgrid():
    GRD_gz = np.reshape(data[2:ps.ADM_kall+2], (ps.ADM_kall))
    GRD_gzh = np.reshape(data[ps.ADM_kall+3:], (ps.ADM_kall))
    # print(GRD_gz)
    # print(GRD_gzh)
